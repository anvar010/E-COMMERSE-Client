...
function ProductListByUser() {
  ...
  const getProductsByCategory = async () => {
    ...
    try {
      let res;
      if (category === 'all') {
        res = await axios.get(`http://localhost:8000/api/v1/product/sellerProducts/${sellerId}`);
      } else {
        res = await axios.get(`http://localhost:8000/api/v1/product/category/${category}`);
      }
      setProduct(res.data.data.products);
      const sellerRes = await axios.get(`http://localhost:8000/api/v1/user/${sellerId}`, {
        headers: {
  ...
  return (
    <div className='pt-[16vh]'>
      <div className='container mx-auto py-8'>
        ...
        {sellerDetails && (
                <h1 className="text-3xl font-bold mb-6">{sellerDetails.name}'s Products</h1>
              )}
